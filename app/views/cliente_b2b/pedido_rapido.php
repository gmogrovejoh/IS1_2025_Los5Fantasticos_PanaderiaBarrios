    ...
    <?php foreach($data['productos'] as $p): ?>
        <div class="col-md-6 col-lg-4 mb-4">
            <div class="card h-100 shadow-sm border-0">
                <div class="text-center bg-light rounded-top p-3" style="min-height: 180px; display: flex; align-items: center; justify-content: center;">
                    <?php if (!empty($p['foto'])): ?>
                        <!-- Si hay foto, la mostramos -->
                        <img src="<?= BASE_URL ?>public/img/<?= $p['foto'] ?>" 
                             class="img-fluid" 
                             alt="<?= $p['nombre'] ?>"
                             style="max-height: 150px; object-fit: contain;">
                    <?php else: ?>
                        <!-- Si no hay foto, mostramos un icono -->
                        <i class="fas fa-bread-slice fa-4x text-muted opacity-25"></i>
                    <?php endif; ?>
                </div>
